...
static void qak_free(qak_allocator* self, void *ptr, const char* sourceFile, int line) {
    qak_allocation_header *header = (qak_allocation_header*)((uint8_t*)ptr - sizeof(qak_allocation_header));
...
void qak_allocator_print(qak_allocator* self) {
    qak_allocation_header *header = self->head;
    while (header) {
        printf("%p, %i bytes %s:%i\n", header, header->numBytes, header->sourceFile, header->line);
        header = header->next;
    }
}